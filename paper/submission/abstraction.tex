...
\begin{theorem}
  ...
  Then there exists a feedback law $\rho_{2}: Q_{1} \times \mathcal{U}_{1} \times \mathcal{D}_{1} \times Q_{2} \to \mathcal{U}_{2}$ %(respectively, $\rho_{2}: Q_{1} \times \mathcal{U}_{1} \times Q_{2} \to \mathcal{U}_{2}$)
  such that for any trajectory of $T_{1}$, %(respectively, $\upsilon_{2}^{k} = \rho_{2} (q_{1}^{k}, \upsilon_{1}^{k}, q_{2}^{k})$)
...
\Cref{thm:ex-simulation-relation} essentially says that if an input-constrained simulation relation $\mathcal{R}$ of $T_{1}$ by $T_2$ exists then $T_{2}$ can be controlled to track any admissible trajectory of $T_{1}$, with respect to $\mathcal{R}$, while keeping the input constraint $\mathcal{R}_{u}$.
