...
\item We developed a scalable hierarchical control structure based on the derived model abstraction, which has a low run-time computational requirement that it can potentially be implemented on embedded computers.  The run-time controller involves only two small-scale linear programs.
