...
We develop in this work a hierarchical design and run-time implementation approach for the %green
...
\begin{figure}[tb]
  ...
  \caption{Overview of the scheduling approach. The blue boxes represent the different models at different scales resulted from the offline design process: the original large-scale binary-input model $\Sigma_{b}$ is reduced to the single-state single-input model $\Sigma_{s}$.  The dashed boxes represent the run-time components, which include two lightweight optimizations and one simple scheduling algorithm.}
  \label{fig:overview}
\end{figure}
...
To reduce the complexity even further, we employ a model abstraction technique based on the concept of simulation relations between transition systems \cite{aluretal00dah,girardetal07amd}.
...
Our approach is best illustrated by \cref{fig:overview}.
