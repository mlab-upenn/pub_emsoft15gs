...
the same input and output as the original model. Under the same input signal, %the reduced model
...
\cref{fig:reduction-vs-abstraction} depicts two models with a symbolic state relation $\mathcal{R}$. % between their states.
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.9\columnwidth]{abstraction_vs_reduction}
\vspace{-6pt}
\caption{\label{fig:reduction-vs-abstraction}Model abstraction (right) is
  different from model order reduction (left): %the models
  $\mathcal{M}_{1}$ and $\mathcal{M}_{2}$ do not share their %the same
  inputs and outputs, but %instead they
...
%The framework of  transition systems allows
...
systems.
As we do not concern with the observable outputs %of the systems
...
\begin{example}
  %The framework of transition systems generalizes
...
Conventionally, simulation relations as defined in \cref{def:simulation-relation} do not explicitly take into account %state constraints and
...
\begin{remark}
  ...
  %In reality, their nondeterminism may be correlated, for example
  if they are subject to the same disturbances in different ways.
  Our extended definition %of simulation relations provides options 
...
%In this section, 
We generalize the label $\sigma$ %of a transition system $T$ 
...
\begin{lemma}
  ...
  Suppose $\mathcal{R}$ is an input-constrained simulation relation of $T_{1}$ by $T_{2}$, %whose initial states satisfy
